...
## Решение
1) Создаем `build`: `mkdir build`. Переходим в неё: `cd build`/
2) Выполняем
3) Выполняем
4) Запускаем программу: `./main` (все должно работать)
5) Тот же самый CMake, но под Windows (код ИИшный и не проверялся, успехов):
```cmake
    # Генерация (находясь в корне проекта)
    cmake -S . -B build
